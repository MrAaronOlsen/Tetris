module Board
  class Shaper
    ...
    def initialize
      @offset = Mat.new_translate(V.new(15.5, 3.5))
    ...
    def draw(scale)
      @next_shape.draw(@offset.add_translate(@shape_offset), scale) if @next_shape

      Render.rect(get_world_sides(@offset, scale), Colors.grey.get, false, 1)
    end
